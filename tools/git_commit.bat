...
:: Ir a la raíz del proyecto (un nivel arriba de tools/)
@echo off
:: Ir a la raíz del proyecto (donde está el .git)
cd /d "%~dp0.."
...
set TAG_CLEAN=%TAG_CLEAN:.= %
for /f "tokens=1,2,3" %%a in ("%TAG_CLEAN%") do (
    set MAJ=%%a
    set MIN=%%b
    set PAT=%%c
)
...
if "%CHANGE_TYPE%"=="1" (
    set /a NEW_PAT=%PAT%+1
    call set NEW_VER=v%MAJ%.%MIN%.%%NEW_PAT%%
)
if "%CHANGE_TYPE%"=="2" (
    set /a NEW_MIN=%MIN%+1
    call set NEW_VER=v%MAJ%.%%NEW_MIN%%.0
)
if "%CHANGE_TYPE%"=="3" (
    set /a NEW_MAJ=%MAJ%+1
    call set NEW_VER=v%%NEW_MAJ%%.0.0
)
if "%CHANGE_TYPE%"=="4" (
